...
const {
    IO: { File, Directory, Path, SearchOption },
    Text: { Encoding },
} = System;
 ...
 */
export function appendTextToFile(
    path: string,
    content: string,
 ...
 */
export function readAllLinesFromFile(
    path: string,
    encoding: any = Encoding.UTF8
): string[] {
 ...
 */
export function readAllTextFromFile(
    path: string,
    encoding: any = Encoding.UTF8
): string {
 ...
 * @param encoding 要使用的字符编码
 */
export function writeAllTextToFile(
    path: string,
    contents?: string,
    encoding: any = Encoding.UTF8
) {
...
export function getFiles(
    ...
    searchOption: any = SearchOption.TopDirectoryOnly
): string[] {
    if (searchPattern)
...
export function getFullPath(path: string, basePath?: string): string {
    if (basePath) return Path.GetFullPath(path, basePath);
